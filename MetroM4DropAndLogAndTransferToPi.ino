...
#define BUFFER_SIZE 64  // UART transfer chunk size
...
void transferCSVtoPi(const char* fileName) {
    ...
    Serial.println("Transferring CSV to PI via UART...");

    uint8_t buffer[BUFFER_SIZE];

    while (file.available()) {
        int bytesRead = file.read(buffer, BUFFER_SIZE);
        Serial1.write(buffer, bytesRead); // Send chunk over UART
    }
...
void logSensorData() {
  ...
  if (dataFile) {
    ...
    if (dataFile.size() == 0) {
      dataFile.println("Timestamp (ms),Total Accel (m/s^2),Peak Acceleration from drop (m/s^2),Impact Duration(ms),Surface Type");
    }
...
void loop() {
  ...
  Serial.println("3 - Drop Sensor, log data, and sent CSV file of data to PI (Simulate Official Drop Test)");
  Serial.println("4 - Transfer A CSV file from Metro M4 to PI");
  ...
  if (action == '1') {
    ...
    Serial.println("Device will begin logging once it has dropped and a CSV file (drop_data_1.csv) will be sent to the PI.");
    Serial.println("Please Ensure PI is set up to receive CSV File, then unplug metro and drop device");
    drop();
    transferCSVtoPi("drop_data_9.csv");
  } else {
