...
void drop() {
  ...
  float totalAccel1 = sqrt((xAcel*xAcel) + (yAcel*yAcel) + (zAcel*zAcel));
  Serial.print("Total Acceleration:"); //dont put spaces serial plotter doesnt recognize
  Serial.print(totalAccel1);
  Serial.println();
  ...
  if (totalAccel1 < freeFallThresh) {
    while (!steadyState) { 
      ...
      if ((impact > 0) && (totalAccel < 8.9)) { //impact detected and at rest i.e steady state
        impactEnd = millis();
